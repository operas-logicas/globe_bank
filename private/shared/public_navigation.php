<?php

// Deafault values to prevent errors
$subject_id = $subject_id ?? '';
$page_id = $page_id ?? '';
$visible = $visible ?? true;

?>

<navigation>

    <?php $nav_subjects = find_all_subjects(['visible' => $visible]); ?>
    <ul class="subjects">
        <?php while($nav_subject = mysqli_fetch_assoc($nav_subjects)) { ?>
             <li class="<?php if($nav_subject['id'] == $subject_id) { echo 'selected'; } ?>">

                <?php
                $nav_pages = find_pages_by_subject_id($nav_subject['id'], ['visible' => $visible]);
                $nav_page = mysqli_fetch_assoc($nav_pages);
                ?>

                <a href="<?php echo url_for('/index.php?id=' . h(u($nav_page['id']))); ?>"><?php echo h($nav_subject['menu_name']); ?></a>
                
                <?php if($nav_subject['id'] == $subject_id) { ?>

                    <ul class="pages">
                        <?php do { // ..while $nav_pages ?>
                            <li class="<?php if($nav_page['id'] == $page_id) { echo 'selected'; } ?>">
                                <a href="<?php echo url_for('/index.php?id=' . h(u($nav_page['id']))); ?>"><?php echo h($nav_page['menu_name']); ?></a>
                            </li><!-- page -->
                        <?php } while($nav_page = mysqli_fetch_assoc($nav_pages)); // end do..while ?>
                    </ul><!-- pages -->

                <?php } // end if $nav_subject['id'] == $subject_id ?>
                <?php mysqli_free_result($nav_pages); ?>

            </li><!-- subject -->
        <?php } // end while $nav_subjects ?>
    </ul><!-- subjects -->
    <?php mysqli_free_result($nav_subjects); ?>

</navigation>